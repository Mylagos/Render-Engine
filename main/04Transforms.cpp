...
namespace gpr5300
{
	...
	void HelloSquare::Begin()
	{
		...
		unsigned char* data = stbi_load("data/textures/zizou_uzumaki.jpg", &width, &height, &nrChannels, 0);
		...
		const auto vertexContent = LoadFile("data/shaders/hello_tutorial/transform.vert");
		const auto* vertexShaderSource = vertexContent.data();

		const auto fragmentContent = LoadFile("data/shaders/hello_tutorial/transform.frag");
		...
		trans_ = glm::translate(trans_, glm::vec3(0.5f, -0.5f, 0.0f));
		trans_ = glm::rotate(trans_, glm::radians(0.0f), glm::vec3(0.0, 0.0, 1.0));
